...
RUN set -eux; \
	chown node:node "$GHOST_ARCH"; 

COPY --chown=node:node ghost/core/ghost-$GHOST_VERSION.tgz $GHOST_ARCH/ghost-$GHOST_VERSION.tgz
...
RUN set -eux; \
	...
	installCmd='gosu node ghost install --archive  $GHOST_ARCH/ghost-$GHOST_VERSION.tgz --db mysql --dbhost mysql --no-prompt --no-stack --no-setup --dir $GHOST_INSTALL'; \	
...
# must be some non-amd64 architecture pre-built binaries aren't published for, so let's install some build deps and do-it-all-over-again
	...
	rm -f $GHOST_ARCH/ghost-$GHOST_VERSION.tgz
