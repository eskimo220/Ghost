...
var headless = require('@lexical/headless');
var list = require('@lexical/list');
var richText = require('@lexical/rich-text');
var link = require('@lexical/link');
var lexical = require('lexical');
var kgDefaultNodes = require('@tryghost/kg-default-nodes');
    ...
    render() {
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            ...
            if (lexical.$isTextNode(node)) {
                ...
                const nextLinkNodeIndex = remainingNodes.findIndex(n => link.$isLinkNode(n));
                ...
                const remainingSortedTextNodes = remainingSortNodes.filter(n => lexical.$isTextNode(n));
                ...
                const nextNode = remainingNodes.find(n => lexical.$isTextNode(n) || link.$isLinkNode(n));
                [...openFormats].forEach((format) => {
                    if (!nextNode || link.$isLinkNode(nextNode) || (nextNode instanceof lexical.TextNode && !nextNode.hasFormat(format))) {
...
function $convertToHtmlString(options = {}) {
    ...
    if (lastChild && lexical.$isParagraphNode(lastChild) && lastChild.getTextContent().trim() === '') {
        output.pop();
    }
    return output.join('');
}
function exportTopLevelElementOrDecorator(node, options) {
    if (kgDefaultNodes.$isKoenigCard(node)) {
    ...
    return lexical.$isElementNode(node) ? exportChildren(node, options) : null;
}
function exportChildren(node, options) {
    ...
    for (const child of children) {
        if (!textContent.isEmpty() && !lexical.$isLineBreakNode(child) && !lexical.$isTextNode(child) && !link.$isLinkNode(child)) {
            output.push(textContent.render());
            textContent.clear();
        }
        if (lexical.$isLineBreakNode(child) || lexical.$isTextNode(child) || link.$isLinkNode(child)) {
...
function getDynamicDataNodes(editorState) {
    ...
    editorState.read(() => {
        ...
        nodes.forEach((node) => {
            if (kgDefaultNodes.$isKoenigCard(node) && node.hasDynamicData?.()) {
...
const useLexicalHtmlRenderer = ({ dom, nodes, onError } = {}) => {
    if (!dom) {
        const { JSDOM } = jsdom;
        dom = new JSDOM();
    }
    ...
    const render = async (lexicalState, userOptions = {}) => {
        ...
        const editor = headless.createHeadlessEditor({
            nodes: DEFAULT_NODES,
            onError: onError
        });
...
exports.useLexicalHtmlRenderer = useLexicalHtmlRenderer;
//# sourceMappingURL=kg-lexical-html-renderer.js.map
