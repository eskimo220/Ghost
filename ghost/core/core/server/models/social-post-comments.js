...
const SocialPostComment = ghostBookshelf.Model.extend({
    ...
    parent() {
        return this.belongsTo('SocialPostComment', 'parent_id');
    },

    inReplyTo() {
        return this.belongsTo('SocialPostComment', 'in_reply_to_id');
    },

    likes() {
        return this.hasMany('SocialCommentLike', 'comment_id');
    },

    replies() {
        return this.hasMany('SocialPostComment', 'parent_id', 'id')
    ...
    applyCustomQuery(options) {
        ...
        this.query((qb) => {
            qb.where(function () {
                this.whereNotIn('social_post_comments.status', excludedCommentStatuses)
                    .orWhereExists(function () {
                        this.select(1)
                            .from('social_post_comments as replies')
                            .whereRaw('replies.parent_id = social_post_comments.id')
    ...
    emitChange: function emitChange(event, options) {
        const eventToTrigger = 'socialpostcomment' + '.' + event;
    ...
    async permissible(commentModelOrId, action, context, unsafeAttrs, loadedPermissions, hasUserPermission, hasApiKeyPermission, hasMemberPermission) {
        ...
        if (action === 'edit' && commentModelOrId.get('user_id') !== context.user.id) {
            return Promise.reject(new errors.NoPermissionError({
                message: tpl(messages.notYourCommentToEdit)
            }));
        }

        if (action === 'destroy' && commentModelOrId.get('user_id') !== context.user.id) {
            return Promise.reject(new errors.NoPermissionError({
        ...
        if (['findAll', 'findPage', 'edit', 'findOne', 'destroy'].indexOf(methodName) !== -1) {
            if (!options.withRelated || options.withRelated.length === 0) {
                if (options.parentId) {
                    ...
                    options.withRelated = [
                        // Relations
                        'inReplyTo', 'user', 'count.likes', 'count.liked'
                    ];
                } else {
                    options.withRelated = [
                        // Relations
                        'user', 'inReplyTo', 'count.replies', 'count.likes', 'count.liked',
                        // Replies (limited to 3)
                        'replies', 'replies.user', 'replies.inReplyTo', 'replies.count.likes', 'replies.count.liked'
                    ];
                }
            }
...
module.exports = {
    SocialPostComment: ghostBookshelf.model('SocialPostComment', SocialPostComment)
};
